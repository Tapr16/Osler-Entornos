/**
 * doctores.js — Lógica CRUD de Doctores
 */
...
function limpiarModal() {
    ['fNombre','fApellido','fLicencia','fEspecialidad','fTurno','fTelefono','fEmail'].forEach(id => {
        const el = document.getElementById(id);
        if (el) el.value = '';
    });
    ['eNombre','eApellido','eLicencia','eEspecialidad'].forEach(id => {
        const el = document.getElementById(id);
...
async function guardar() {
    ...
    try {
        ...
        if (res.ok) {
            cerrarModal();
            showGlobalAlert(id ? 'Doctor actualizado ✅' : 'Doctor creado ✅', 'success');
            cargarDoctores();
        } else {
