...
- **Agenda Médica**: Visualización de turnos mediante calendario interactivo colapsable.
- **Historial Clínico**: Registro de consultas, diagnósticos y tratamientos desde cada turno.
- **Control de Turnos**: Gestión de estados (Programada, En curso, Completada, Cancelada).

### 👤 Dashboard del Paciente (Especializado)

- **Mis Citas**: Seguimiento de citas pendientes y consultas pasadas.
- **Calendario Interactivo**: Vista mensual de citas con filtro por estado y detalle por día.
...
│       └── routes/             # Endpoints REST (auth, pacientes, doctores, citas, historial)
│
├── frontend-react/
│   └── src/
│       ├── api/                # Instancia Axios con interceptores JWT
│       ├── components/         # Sidebar, ProtectedRoute, PerfilModal, CalendarioCitas
│       ├── context/            # AuthContext (estado global de sesión)
│       └── pages/              # Login, Register, Dashboard, Pacientes, Doctores, Citas,
│                               # DashboardDoctor, DashboardPaciente
│
└── base-de-datos/
    └── osler-db.sql            # Script de creación e inserción de datos iniciales
```
